// @arc-lang/qrcode — Zero-dependency QR code generator
// Inline SVG output, single <path> element, horizontal run-length encoding.
...
let _j = 255;
while (_j < 512) { exp[_j] = exp[_j - 255]; _j += 1; }
...
function encode(text, ecLvl) {
  ...
  function pushBits(val, n) {
    ...
    while (i >= 0) {
      acc = (acc << 1) | ((val >> i) & 1);
      accN += 1;
      if (accN == 8) { cws.push(acc & 255); acc = 0; accN = 0; }
      i -= 1;
    }
  }
  ...
  let pi = 0;
  while (cws.length < totalData) { cws.push(pi % 2 == 0 ? 236 : 17); pi += 1; }
...
function buildMatrix(cws, ver) {
  const sz = 17 + 4 * ver;
  const mod = new Uint8Array(sz * sz);
  const res = new Uint8Array(sz * sz);

  function set(x, y, v) { mod[y * sz + x] = v; res[y * sz + x] = 1; }
  ...
  if (ver >= 7) {
    ...
    let vi = 17;
    while (vi >= 12) {
      if ((vinfo >> vi) & 1) { vinfo = vinfo ^ (7973 << (vi - 12)); }
      vi -= 1;
    }
...
function applyMask(mod, res, sz, mask) {
...
function writeFmt(mod, sz, ecLvl, mask) {
  const ecBits = [1, 0, 3, 2];
  let data = (ecBits[ecLvl] << 3) | mask;
  let rem = data << 10;
  let i = 14;
  while (i >= 10) {
    if ((rem >> i) & 1) { rem = rem ^ (1335 << (i - 10)); }
    i -= 1;
  }
 ...
 * @param {number} [options.size=200] - Rendered pixel size.
 * @param {string} [options.dark='#000000'] - Dark module color.
 * @param {string} [options.light='#ffffff'] - Light (background) color.
 * @param {'L'|'M'|'Q'|'H'} [options.level='M'] - Error correction level.
 * @returns {string} SVG string, or empty string for empty input / overflow.
 */
export function toSvg(text, options = {}) {
  if (!text || text.length == 0) { return ''; }
  const { size = 200, dark = '#000000', light = '#ffffff', level = 'M' } = options;
  const safeSize = Math.max(10, Math.min(4096, Number(size) || 200));
  const safeDark = String(dark || '#000000').replace(/[^a-zA-Z0-9#().,\s%-]/g, '');
  const safeLight = String(light || '#ffffff').replace(/[^a-zA-Z0-9#().,\s%-]/g, '');
  const ecMap = { L: 0, M: 1, Q: 2, H: 3 };
  const ecLvl = ecMap[String(level).toUpperCase()] ?? 1;
